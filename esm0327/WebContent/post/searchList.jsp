...
<%-- 		<form name="searchForm" action="${pageContext.request.contextPath}/dispatcher"> 
	...
	<ul class="pagination pagination">
		...
		<c:if test="${pb.previousPageGroup}">		
			<li><a href="dispatcher?command=Search&pageNo=${pb.startPageOfPageGroup-1}">«</a></li>
		</c:if>
		<%--페이지그룹 시작번호부터 끝번호 및 현재 페이지 --%>
		<c:forEach var="page" begin="${pb.startPageOfPageGroup}" end="${pb.endPageOfPageGroup}">
		<c:choose>
			<c:when test="${pb.nowPage!=page}">
				<li><a href="dispatcher?command=Search&pageNo=${page}">${page}</a></li>
			</c:when>
			<c:otherwise>
		...
		<c:if test="${pb.nextPageGroup}">		
			<li><a href="dispatcher?command=Search&pageNo=${pb.endPageOfPageGroup+1}">»</a></li>
		</c:if>
	</ul>
</div>
